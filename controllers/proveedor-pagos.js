...
const getAll2 = async(req, res) => {
    ...
    try {
        ...
        //todo: entidades financieras
        q3 = `SELECT * FROM TBL_ENTIDADES_FINANCIERAS WHERE HABILITADO = 1`;
        //? mapear y camelcase
...
const getById = async(req, res) => {
    ...
    try {
        ...
        const result = await consulta(q);
        if (result.length == 0) return null;
        //? consulta de muchos a muchos tabla TBL_DEC_MEDIO_PAGO_ENT_FINANC
        ...
        if (result3.length == 0) return null;
        //? mapear y camelcase
        const data = result.map(item => {
            return {
                habilitado: item.HABILITADO,
